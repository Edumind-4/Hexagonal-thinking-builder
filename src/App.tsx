...
export default function App() {
  ...
  const exportImage = async () => {
    ...
    if (scrollContainer) {
      // Calculate height needed to fit all cards
      const requiredHeight = Math.max(mainRef.current.clientHeight, scrollContainer.scrollHeight + 100);
  ...
  return (
    <div className="flex flex-col h-screen max-h-screen bg-slate-900 font-sans">
      {/* Header */}
      <header className="h-16 px-6 border-b border-slate-700 bg-slate-800-80 flex items-center justify-between z-50 shrink-0">
        <div className="flex items-center gap-4">
          <div className="w-10 h-10 bg-indigo-500 rounded flex items-center justify-center safe-shadow-lg">
            <Hexagon className="w-6 h-6 text-white" />
          </div>
          <div>
            <h1 className="text-lg font-bold tracking-tight text-white leading-none">Hexagonal Thinking Builder</h1>
            <p className="text-[11px] text-slate-400 mt-0.5 uppercase tracking-wider font-medium">Critical Thinking Framework</p>
          </div>
        </div>

        <div className="flex items-center gap-4">
          <form onSubmit={addNode} className="flex items-center" data-html2canvas-ignore="true">
            <input
              type="text"
              value={inputValue}
              onChange={(e) => setInputValue(e.target.value)}
              placeholder="Add concept..."
              className="bg-slate-700 border border-slate-600 rounded-l py-2 px-4 text-sm w-48 outline-none focus:ring-1 ring-indigo-400 text-white placeholder:text-slate-500 transition-all focus:w-64"
            />
            <button
              type="submit"
              className="bg-indigo-600 hover:bg-indigo-500 text-white px-4 py-2 rounded-r text-sm font-semibold transition-colors flex items-center gap-2 safe-shadow-inner"
            >
              Add Node
            </button>
          </form>

          <div className="w-px h-8 bg-slate-700 mx-2" data-html2canvas-ignore="true" />

          <button
            onClick={exportImage}
            className="bg-slate-100 hover:bg-white text-slate-900 px-4 py-2 rounded text-sm font-bold flex items-center gap-2 transition-all safe-shadow-sm active:scale-95"
      ...
      <main ref={mainRef} className="flex-1 flex overflow-hidden relative">
        ...
        <div 
          ref={canvasRef}
          className="flex-1 relative grid-bg overflow-hidden cursor-crosshair bg-slate-900"
        >
          ...
          {nodes.map((node) => (
            ...
            >
              ...
              <div
                ...
                className={`w-32 h-32 flex items-center justify-center relative transition-transform cursor-grab active:cursor-grabbing`}
              >
              ...
              {/* Hover Actions */}
              <div 
                className="absolute -top-2 -right-2 flex gap-1 opacity-0 group-hover:opacity-100 transition-all z-30"
                data-html2canvas-ignore="true"
              >
                <button
          ...
          {/* Canvas Instructions Placeholder */}
          {nodes.length === 0 && (
            <div className="absolute inset-0 flex items-center justify-center pointer-events-none opacity-10 flex-col gap-4">
              <Hexagon className="w-32 h-32 text-slate-400" />
              <p className="text-xl font-bold uppercase tracking-widest text-slate-400">Concept Map Canvas</p>
            </div>
          )}
        </div>

        {/* Right Column: Justification Panel */}
        <aside className="w-80 bg-slate-800 border-l border-slate-700 flex flex-col shrink-0 z-40 relative">
          <div className="p-4 border-b border-slate-700 flex justify-between items-center bg-slate-800-40 shrink-0">
            <h2 className="text-sm font-bold uppercase tracking-widest text-slate-400 whitespace-nowrap">Justification Panel</h2>
            {links.length > 0 && (
              <span className="px-2 py-0.5 bg-indigo-900 text-indigo-200 text-[10px] rounded-full font-bold">{links.length} Links</span>
            )}
          </div>

          <div className="flex-1 overflow-y-auto p-4 custom-scrollbar space-y-4 bg-slate-800-20">
            <AnimatePresence>
              {links.map((link) => {
                const fromNode = nodes.find(n => n.id === link.fromId);
                const toNode = nodes.find(n => n.id === link.toId);
            ...
            {links.length === 0 && (
              <div className="h-full flex items-center justify-center text-slate-500 text-[11px] italic text-center p-8 uppercase tracking-widest leading-loose opacity-50">
            ...
            {isExporting && (
              <div className="pt-8 pb-4 text-center">
                <p className="text-[10px] text-slate-500 uppercase tracking-widest leading-loose">
                  Created at https://cbse.smartresourcesacademy.com/hexagonal-thinking-builder<br/>
                  Digital Curriculum Sandbox
                </p>
              </div>
            )}
          </div>
        ...
        {!isExporting && (
          <footer className="absolute bottom-0 left-0 right-0 h-8 bg-slate-950 px-4 flex items-center justify-center shrink-0 border-t border-slate-800 z-50">
